...
export default function PackageRecommender({ packageOne, packageTwo }) {
    ...
    useEffect(() => {
        ...
        if (scoreOne > scoreTwo) {
            ...
            setTimesBetter(scoreTwo/scoreOne);
    ...
    return (
        ...
        <Stack sx={{marginTop:8}}>
            ...
            <Paper elevation={5} square={true} sx={{
                padding: 3,
                width:"65%",
                marginLeft:'auto',
                marginRight:'auto',
            ...
            <a href={packageData.collected.metadata.links.npm}>HomePage</a>
        </Typography>
                </Stack>
                <Stack direction={'row'} sx={{width:"50%"}}>
                    ...
                    <Typography variant='h6'>  {`${Math.round(packageData.evaluation.quality.tests)}%`} </Typography>
                    </Paper>
                </Stack>
                </Stack>
                <Stack>
